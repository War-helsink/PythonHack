...
def argument():
    ...
    if not options.ip and not options.interface:
        p.error()
    else:
        return options
...
def restore(ip, mac, ip_hack, mac_hack):
...
try:
    ...
    restore(f[b]["ip"], f[b]["mac"], f[a]["ip"], f[a]["mac"])
    restore(f[a]["ip"], f[a]["mac"], f[b]["ip"], f[b]["mac"])