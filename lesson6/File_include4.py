...
def process_packet(packet):
    ...
    if ip_packet.haslayer(scapy.Raw):
        if ip_packet[scapy.TCP].dport == 80 and "192.168.0.1" not in ip_packet[scapy.Raw].load:
            if ".exe" == ip_packet[scapy.Raw].load:
                print("[+] HTTP Request port 80, exe file.")
                ack_list.append(ip_packet[scapy.TCP].ack)
        elif ip_packet[scapy.TCP].sport == 80 and "192.168.0.1" not in ip_packet[scapy.Raw].load:
